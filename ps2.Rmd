...
```{r}
setwd("~/Downloads/Psych251/psych251_problem_sets")
...
```{r}
unique(rating_hyp_long_d$Measurement)
...
performance_diff_d = performance_hyp_long_d %>%
  ...
  #if leaving them in, in measurement the with music is post music, and the without music is pre music)
...
performance_diff_summary_d = performance_diff_d %>%
  ...
  summarize(MeanImprovementScore = mean(ImprovementScore), 
            SEMImprovementScore = sd(ImprovementScore)/sqrt(length(ImprovementScore)))
